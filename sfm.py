...
def main():
  ...
  for pose in possible_relative_poses:
    # e_im2.SetPose(id_R, id_t)
    # e_im1.SetPose(pose[0],pose[1])

    # _, im1_pts, _ = TriangulatePoints(K, e_im1, e_im2, e_matches)
    # ct = im1_pts.shape[0]
    print(max_ct)
    
    e_im1.SetPose(id_R, id_t)
    e_im2.SetPose(pose[0],pose[1])

    _, _, im2_pts = TriangulatePoints(K, e_im1, e_im2, e_matches)
    ct = im2_pts.shape[0]
  ...
  e_im1.SetPose(max_pose[0], max_pose[1])
  e_im2.SetPose(max_pose[0], max_pose[1])
